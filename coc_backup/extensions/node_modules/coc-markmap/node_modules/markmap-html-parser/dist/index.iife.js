...
(function(exports) {
  ...
  function parseHtml(html2, opts) {
    ...
    checkNode($root);
    walkTree(rootNode, (node, next2) => {
      next2();
      const $el = elMap[node.id];
      $el.contents().each((_, child) => {
        if (child.type === "comment") {
          const data22 = child.data.trim();
          if (data22.startsWith(MARKMAP_COMMENT_PREFIX)) {
            node.comments || (node.comments = []);
            node.comments.push(data22.slice(MARKMAP_COMMENT_PREFIX.length).trim());
            $2(child).remove();
          }
        }
      });
      ...
      node.html = (($el.is(options.selectorPreserveTag) ? $2.html($el) : $el.html()) || "").trimEnd();
    });
    return rootNode;
    function addChild($child, parent2, nesting) {
      const child = $child[0];
      const node = {
        id: ++id,
        tag: child.tagName,
...
})(this.markmap.htmlParser = this.markmap.htmlParser || {});
