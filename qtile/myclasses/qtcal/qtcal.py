...
class QtCalWindow(QMainWindow):
    ...
    def showAppointment(self, date):
        ...
        selectedDate = str(date.getDate()[0]) + "-" + str(date.getDate()[1]).zfill(2) + "-" + str(date.getDate()[2]).zfill(2)
        
        #fill list with tracking entries
        vSqlResults = selectDbData(self.sqlCon, """SELECT * from appointments where date = ? order by date, starttime asc""", (selectedDate,))
        self.lEntries = []
